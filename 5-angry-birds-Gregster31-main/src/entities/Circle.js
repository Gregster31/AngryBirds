...
export default class Circle extends GameEntity {
	 ...
	 */
	constructor(x, y, radius, options, birdType) {
	...
	didGoOffScreen() {
		let screen = CANVAS_WIDTH
		if(this.type == BirdType.White || this.type == BirdType.Blue) {
			screen = 2400
		}
		return this.body.position.x + this.radius < 0 || this.body.position.x - this.radius > screen;
	}
